...
export const cartItemHandlers = [
    ...
    http.patch("/plant/cart_items/:id", async ({ params, request }) => {
        ...
        item.price = getFinalPrice(product, body.quantity);
    ...
    http.post("/plant/cart_items", async ({ request }) => {
        ...
        const finalPrice = getFinalPrice(product, body.quantity);

        if (existed) {
            existed.quantity += body.quantity;
            existed.price = getFinalPrice(product, existed.quantity);
